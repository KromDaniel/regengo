...
//	    })
//	io.Copy(os.Stdout, r)
func (TDFALogParser) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *TDFALogParserBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return TDFALogParser{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (TDFALogParser) processTransform(data []byte, isEOF bool, onMatch func(match *TDFALogParserBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &TDFALogParserBytesResult{}

	for {
		result, ok := TDFALogParser{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (TDFALogParser) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &tDFALogParserTransformErrReader{err: err}
	}

	captureNames := map[string]int{"timestamp": 1, "ms": 2, "tz": 3, "level": 4, "message": 5}
	resolved, err := parsed.ValidateAndResolve(captureNames, 5)
	if err != nil {
		return &tDFALogParserTransformErrReader{err: err}
	}

	return TDFALogParser{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *TDFALogParserBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (TDFALogParser{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// tDFALogParserTransformErrReader is a reader that always returns an error.
type tDFALogParserTransformErrReader struct {
	err error
}

func (r *tDFALogParserTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (TDFALogParser) getCaptureByIndex(m *TDFALogParserBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	case 1:
...
//	})
func (TDFALogParser) SelectReader(r io.Reader, pred func(*TDFALogParserBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return TDFALogParser{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (TDFALogParser) processSelect(data []byte, isEOF bool, pred func(*TDFALogParserBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &TDFALogParserBytesResult{}
...
//	})
func (TDFALogParser) RejectReader(r io.Reader, pred func(*TDFALogParserBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return TDFALogParser{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (TDFALogParser) processReject(data []byte, isEOF bool, pred func(*TDFALogParserBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &TDFALogParserBytesResult{}
...
// TDFALogParserReplaceTemplate holds a pre-compiled replace template for the TDFALogParser pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type TDFALogParserReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (TDFALogParser) CompileReplaceTemplate(template string) (*TDFALogParserReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &TDFALogParserReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *TDFALogParserReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *TDFALogParserReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r TDFALogParserResult
...
func (t *TDFALogParserReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *TDFALogParserReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r TDFALogParserBytesResult
...
func (t *TDFALogParserReplaceTemplate) ReplaceFirstString(input string) string {
	var r TDFALogParserResult
	match, ok := TDFALogParser{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *TDFALogParserReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r TDFALogParserBytesResult
	match, ok := TDFALogParser{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
