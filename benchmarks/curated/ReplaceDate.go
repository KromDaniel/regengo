...
//	    })
//	io.Copy(os.Stdout, r)
func (ReplaceDate) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReplaceDateBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReplaceDate{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReplaceDate) processTransform(data []byte, isEOF bool, onMatch func(match *ReplaceDateBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReplaceDateBytesResult{}

	for {
		result, ok := ReplaceDate{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReplaceDate) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &replaceDateTransformErrReader{err: err}
	}
	...
	return ReplaceDate{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReplaceDateBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReplaceDate{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// replaceDateTransformErrReader is a reader that always returns an error.
type replaceDateTransformErrReader struct {
	err error
}

func (r *replaceDateTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReplaceDate) getCaptureByIndex(m *ReplaceDateBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	case 1:
...
//	})
func (ReplaceDate) SelectReader(r io.Reader, pred func(*ReplaceDateBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1024

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReplaceDate{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReplaceDate) processSelect(data []byte, isEOF bool, pred func(*ReplaceDateBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReplaceDateBytesResult{}
...
//	})
func (ReplaceDate) RejectReader(r io.Reader, pred func(*ReplaceDateBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1024

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReplaceDate{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReplaceDate) processReject(data []byte, isEOF bool, pred func(*ReplaceDateBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReplaceDateBytesResult{}
...
// ReplaceDateReplaceTemplate holds a pre-compiled replace template for the ReplaceDate pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReplaceDateReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReplaceDate) CompileReplaceTemplate(template string) (*ReplaceDateReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReplaceDateReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReplaceDateReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReplaceDateReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReplaceDateResult
...
func (t *ReplaceDateReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReplaceDateReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReplaceDateBytesResult
...
func (t *ReplaceDateReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReplaceDateResult
	match, ok := ReplaceDate{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReplaceDateReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReplaceDateBytesResult
	match, ok := ReplaceDate{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
