package curated

import (
	"fmt"
	stream "github.com/KromDaniel/regengo/stream"
	"regexp"
	"strings"
	"testing"
)

// Code generated by regengo
// Tests and benchmarks for pattern: (?P<user>[\w\.+-]+)@(?P<domain>[\w\.-]+)\.(?P<tld>[\w\.-]+)

var replaceEmailRegexp = regexp.MustCompile("(?P<user>[\\w\\.+-]+)@(?P<domain>[\\w\\.-]+)\\.(?P<tld>[\\w\\.-]+)")

var replaceEmailTestInputs = []string{"Contact support@example.com for help", "Multiple: a@b.com", "c@d.org", "e@f.net in one line", "user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com user@example.com"}

func TestReplaceEmailMatchString(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		expected := replaceEmailRegexp.MatchString(input)
		got := ReplaceEmail{}.MatchString(input)
		if got != expected {
			t.Errorf("MatchString(%q) = %v, want %v", input, got, expected)
		}
	}
}

func TestReplaceEmailMatchBytes(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		inputBytes := []byte(input)
		expected := replaceEmailRegexp.Match(inputBytes)
		got := ReplaceEmail{}.MatchBytes(inputBytes)
		if got != expected {
			t.Errorf("MatchBytes(%q) = %v, want %v", input, got, expected)
		}
	}
}

func TestReplaceEmailFindString(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		expectedMatches := replaceEmailRegexp.FindStringSubmatch(input)
		result, ok := ReplaceEmail{}.FindString(input)

		if len(expectedMatches) > 0 {
			if !ok {
				t.Errorf("FindString(%q) returned false, expected match", input)
				continue
			}
			if result.Match != expectedMatches[0] {
				t.Errorf("FindString(%q).Match = %q, want %q", input, result.Match, expectedMatches[0])
			}
			{
				if result.User != expectedMatches[1] {
					t.Errorf("FindString(%q).User = %q, want %q", input, result.User, expectedMatches[1])
				}
				if result.Domain != expectedMatches[2] {
					t.Errorf("FindString(%q).Domain = %q, want %q", input, result.Domain, expectedMatches[2])
				}
				if result.Tld != expectedMatches[3] {
					t.Errorf("FindString(%q).Tld = %q, want %q", input, result.Tld, expectedMatches[3])
				}
			}
		} else {
			if ok {
				t.Errorf("FindString(%q) returned true, expected no match", input)
			}
		}
	}
}

func TestReplaceEmailFindAllString(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		expectedAll := replaceEmailRegexp.FindAllStringSubmatch(input, -1)
		results := ReplaceEmail{}.FindAllString(input, -1)

		if len(results) != len(expectedAll) {
			t.Errorf("FindAllString(%q, -1) returned %d results, want %d", input, len(results), len(expectedAll))
			continue
		}
		for i := range results {
			if results[i].Match != expectedAll[i][0] {
				t.Errorf("FindAllString(%q, -1)[%d].Match = %q, want %q", input, i, results[i].Match, expectedAll[i][0])
			}
			{
				if results[i].User != expectedAll[i][1] {
					t.Errorf("FindAllString(%q, -1)[%d].User = %q, want %q", input, i, results[i].User, expectedAll[i][1])
				}
				if results[i].Domain != expectedAll[i][2] {
					t.Errorf("FindAllString(%q, -1)[%d].Domain = %q, want %q", input, i, results[i].Domain, expectedAll[i][2])
				}
				if results[i].Tld != expectedAll[i][3] {
					t.Errorf("FindAllString(%q, -1)[%d].Tld = %q, want %q", input, i, results[i].Tld, expectedAll[i][3])
				}
			}
		}
	}
}

func BenchmarkReplaceEmail(b *testing.B) {
	inputs := replaceEmailTestInputs

	b.Run("Match", func(b *testing.B) {
		for i, input := range inputs {
			input := input
			b.Run(fmt.Sprintf("Input[%d]", i), func(b *testing.B) {
				b.Run("stdlib", func(b *testing.B) {
					b.ReportAllocs()
					for b.Loop() {
						_ = replaceEmailRegexp.MatchString(input)
					}
				})
				b.Run("regengo", func(b *testing.B) {
					b.ReportAllocs()
					for b.Loop() {
						_ = ReplaceEmail{}.MatchString(input)
					}
				})
			})
		}
	})

	b.Run("FindFirst", func(b *testing.B) {
		for i, input := range inputs {
			input := input
			b.Run(fmt.Sprintf("Input[%d]", i), func(b *testing.B) {
				b.Run("stdlib", func(b *testing.B) {
					b.ReportAllocs()
					for b.Loop() {
						_ = replaceEmailRegexp.FindStringSubmatch(input)
					}
				})
				b.Run("regengo", func(b *testing.B) {
					b.ReportAllocs()
					for b.Loop() {
						_, _ = ReplaceEmail{}.FindString(input)
					}
				})
				b.Run("regengo_reuse", func(b *testing.B) {
					b.ReportAllocs()
					var result *ReplaceEmailResult
					for b.Loop() {
						result, _ = ReplaceEmail{}.FindStringReuse(input, result)
					}
				})
			})
		}
	})

	b.Run("FindAll", func(b *testing.B) {
		for i, input := range inputs {
			input := input
			b.Run(fmt.Sprintf("Input[%d]", i), func(b *testing.B) {
				b.Run("stdlib", func(b *testing.B) {
					b.ReportAllocs()
					for b.Loop() {
						_ = replaceEmailRegexp.FindAllStringSubmatch(input, -1)
					}
				})
				b.Run("regengo", func(b *testing.B) {
					b.ReportAllocs()
					for b.Loop() {
						_ = ReplaceEmail{}.FindAllString(input, -1)
					}
				})
				b.Run("regengo_append", func(b *testing.B) {
					b.ReportAllocs()
					results := make([]*ReplaceEmailResult, 0, 100)
					for b.Loop() {
						results = ReplaceEmail{}.FindAllStringAppend(input, -1, results[:0])
					}
				})
			})
		}
	})

	b.Run("Replace", func(b *testing.B) {
		for j, template := range replaceEmailTestReplacers {
			template := template
			stdlibTemplate := convertToStdlibTemplateReplaceEmail(template)
			b.Run(fmt.Sprintf("Template[%d]", j), func(b *testing.B) {
				for i, input := range inputs {
					input := input
					inputBytes := []byte(input)
					j := j
					b.Run(fmt.Sprintf("Input[%d]", i), func(b *testing.B) {
						b.Run("stdlib", func(b *testing.B) {
							b.ReportAllocs()
							for b.Loop() {
								_ = replaceEmailRegexp.ReplaceAllString(input, stdlibTemplate)
							}
						})
						b.Run("regengo_runtime", func(b *testing.B) {
							b.ReportAllocs()
							for b.Loop() {
								_ = ReplaceEmail{}.ReplaceAllString(input, template)
							}
						})
						b.Run("regengo", func(b *testing.B) {
							b.ReportAllocs()
							for b.Loop() {
								switch j {
								case 0:
									_ = ReplaceEmail{}.ReplaceAllString0(input)
								case 1:
									_ = ReplaceEmail{}.ReplaceAllString1(input)
								case 2:
									_ = ReplaceEmail{}.ReplaceAllString2(input)
								}
							}
						})
						b.Run("regengo_append", func(b *testing.B) {
							b.ReportAllocs()
							buf := make([]byte, 0, 4096)
							for b.Loop() {
								switch j {
								case 0:
									buf = ReplaceEmail{}.ReplaceAllBytesAppend0(inputBytes, buf[:0])
								case 1:
									buf = ReplaceEmail{}.ReplaceAllBytesAppend1(inputBytes, buf[:0])
								case 2:
									buf = ReplaceEmail{}.ReplaceAllBytesAppend2(inputBytes, buf[:0])
								}
							}
						})
					})
				}
			})
		}
	})

}

func TestReplaceEmailFindReader(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		// Get expected matches from stdlib
		expectedAll := replaceEmailRegexp.FindAllString(input, -1)

		// Get matches from streaming API
		var streamMatches []string
		err := ReplaceEmail{}.FindReader(strings.NewReader(input), stream.Config{}, func(m stream.Match[*ReplaceEmailBytesResult]) bool {
			streamMatches = append(streamMatches, string(m.Result.Match))
			return true
		})
		if err != nil {
			t.Errorf("FindReader(%q) error: %v", input, err)
			continue
		}

		// Compare results
		if len(streamMatches) != len(expectedAll) {
			t.Errorf("FindReader(%q) got %d matches, want %d", input, len(streamMatches), len(expectedAll))
			continue
		}
		for i := range streamMatches {
			if streamMatches[i] != expectedAll[i] {
				t.Errorf("FindReader(%q)[%d] = %q, want %q", input, i, streamMatches[i], expectedAll[i])
			}
		}
	}
}

func TestReplaceEmailFindReaderCount(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		expectedAll := replaceEmailRegexp.FindAllString(input, -1)
		expected := int64(len(expectedAll))

		count, err := ReplaceEmail{}.FindReaderCount(strings.NewReader(input), stream.Config{})
		if err != nil {
			t.Errorf("FindReaderCount(%q) error: %v", input, err)
			continue
		}
		if count != expected {
			t.Errorf("FindReaderCount(%q) = %d, want %d", input, count, expected)
		}
	}
}

func TestReplaceEmailFindReaderFirst(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		expected := replaceEmailRegexp.FindString(input)

		result, _, err := ReplaceEmail{}.FindReaderFirst(strings.NewReader(input), stream.Config{})
		if err != nil {
			t.Errorf("FindReaderFirst(%q) error: %v", input, err)
			continue
		}

		if expected == "" {
			if result != nil {
				t.Errorf("FindReaderFirst(%q) = %q, want no match", input, string(result.Match))
			}
		} else {
			if result == nil {
				t.Errorf("FindReaderFirst(%q) = nil, want %q", input, expected)
			} else if string(result.Match) != expected {
				t.Errorf("FindReaderFirst(%q) = %q, want %q", input, string(result.Match), expected)
			}
		}
	}
}

func TestReplaceEmailMatchLengthInfo(t *testing.T) {
	minLen, maxLen := ReplaceEmail{}.MatchLengthInfo()
	// Just verify we get sensible values
	if minLen < 0 {
		t.Errorf("MinMatchLen = %d, expected >= 0", minLen)
	}
	if maxLen != -1 && maxLen < minLen {
		t.Errorf("MaxMatchLen = %d < MinMatchLen = %d", maxLen, minLen)
	}
}

var replaceEmailTestReplacers = []string{"$user@REDACTED.$tld", "[EMAIL]", "$0"}

// convertToStdlibTemplate converts regengo template syntax to stdlib syntax.
// regengo: $name, $1, $0 -> stdlib: ${name}, ${1}, ${0}
func convertToStdlibTemplateReplaceEmail(template string) string {
	var result strings.Builder
	i := 0
	for i < len(template) {
		if template[i] != '$' {
			result.WriteByte(template[i])
			i++
			continue
		}
		// Found $
		if i+1 >= len(template) {
			result.WriteByte('$')
			i++
			continue
		}
		next := template[i+1]
		switch {
		case next == '$':
			// $$ -> $
			result.WriteString("$$")
			i += 2
		case next == '{':
			// ${...} -> ${...} (already in stdlib format)
			closeIdx := strings.Index(template[i:], "}")
			if closeIdx == -1 {
				result.WriteByte('$')
				i++
			} else {
				result.WriteString(template[i : i+closeIdx+1])
				i += closeIdx + 1
			}
		case next >= '0' && next <= '9':
			// $0, $1, etc -> ${0}, ${1}, etc
			j := i + 1
			for j < len(template) && template[j] >= '0' && template[j] <= '9' {
				j++
			}
			result.WriteString("${")
			result.WriteString(template[i+1 : j])
			result.WriteByte('}')
			i = j
		case next == '_' || (next >= 'a' && next <= 'z') || (next >= 'A' && next <= 'Z'):
			// $name -> ${name}
			j := i + 1
			for j < len(template) && (template[j] == '_' || (template[j] >= 'a' && template[j] <= 'z') || (template[j] >= 'A' && template[j] <= 'Z') || (template[j] >= '0' && template[j] <= '9')) {
				j++
			}
			result.WriteString("${")
			result.WriteString(template[i+1 : j])
			result.WriteByte('}')
			i = j
		default:
			// Lone $ followed by something else
			result.WriteByte('$')
			i++
		}
	}
	return result.String()
}

func TestReplaceEmailReplaceAllString(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		for _, replacer := range replaceEmailTestReplacers {
			stdlibTemplate := convertToStdlibTemplateReplaceEmail(replacer)
			expected := replaceEmailRegexp.ReplaceAllString(input, stdlibTemplate)
			got := ReplaceEmail{}.ReplaceAllString(input, replacer)
			if got != expected {
				t.Errorf("ReplaceAllString(%q, %q) = %q, want %q", input, replacer, got, expected)
			}
		}
	}
}

func TestReplaceEmailReplaceFirstString(t *testing.T) {
	for _, input := range replaceEmailTestInputs {
		for _, replacer := range replaceEmailTestReplacers {
			// Simulate ReplaceFirst using ReplaceAllStringFunc with a once flag
			stdlibTemplate := convertToStdlibTemplateReplaceEmail(replacer)
			replaced := false
			expected := replaceEmailRegexp.ReplaceAllStringFunc(input, func(match string) string {
				if replaced {
					return match
				}
				replaced = true
				// Use Expand to apply template to single match
				submatch := replaceEmailRegexp.FindStringSubmatchIndex(match)
				if submatch == nil {
					return match
				}
				var dst []byte
				dst = replaceEmailRegexp.ExpandString(dst, stdlibTemplate, match, submatch)
				return string(dst)
			})
			got := ReplaceEmail{}.ReplaceFirstString(input, replacer)
			if got != expected {
				t.Errorf("ReplaceFirstString(%q, %q) = %q, want %q", input, replacer, got, expected)
			}
		}
	}
}

func TestReplaceEmailReplaceAllString0(t *testing.T) {
	replacer := "$user@REDACTED.$tld"
	for _, input := range replaceEmailTestInputs {
		runtime := ReplaceEmail{}.ReplaceAllString(input, replacer)
		precompiled := ReplaceEmail{}.ReplaceAllString0(input)
		if precompiled != runtime {
			t.Errorf("ReplaceAllString0(%q) = %q, want %q (runtime)", input, precompiled, runtime)
		}
	}
}

func TestReplaceEmailReplaceAllString1(t *testing.T) {
	replacer := "[EMAIL]"
	for _, input := range replaceEmailTestInputs {
		runtime := ReplaceEmail{}.ReplaceAllString(input, replacer)
		precompiled := ReplaceEmail{}.ReplaceAllString1(input)
		if precompiled != runtime {
			t.Errorf("ReplaceAllString1(%q) = %q, want %q (runtime)", input, precompiled, runtime)
		}
	}
}

func TestReplaceEmailReplaceAllString2(t *testing.T) {
	replacer := "$0"
	for _, input := range replaceEmailTestInputs {
		runtime := ReplaceEmail{}.ReplaceAllString(input, replacer)
		precompiled := ReplaceEmail{}.ReplaceAllString2(input)
		if precompiled != runtime {
			t.Errorf("ReplaceAllString2(%q) = %q, want %q (runtime)", input, precompiled, runtime)
		}
	}
}
