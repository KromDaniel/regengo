...
var emailCaptureCaptureStackPool = sync.Pool{New: func() interface{} {
...
const EmailCaptureMinMatchLen = 5

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const EmailCaptureMaxMatchLen = -1

func (EmailCapture) MatchLengthInfo() (minLen, maxLen int) {
	return EmailCaptureMinMatchLen, EmailCaptureMaxMatchLen
}

func (EmailCapture) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := emailCaptureStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
...
type EmailCaptureResult struct {
...
func (EmailCapture) FindStringReuse(input string, r *EmailCaptureResult) (*EmailCaptureResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	...
	defer func() {
		*captureStackPtr = captureStack[:0]
		emailCaptureCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := emailCaptureStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
func (recv EmailCapture) FindString(input string) (*EmailCaptureResult, bool) {
...
func (EmailCapture) FindBytesReuse(input []byte, r *EmailCaptureBytesResult) (*EmailCaptureBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := emailCaptureCaptureStackPool.Get().(*[]int)
...
func (recv EmailCapture) FindBytes(input []byte) (*EmailCaptureBytesResult, bool) {
...
func (EmailCapture) DefaultMaxLeftover() int {
	return 1048576
}
...
func (EmailCapture) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*EmailCaptureBytesResult]) bool) error {
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						...
						m := stream.Match[*EmailCaptureBytesResult]{
...
func (EmailCapture) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := EmailCapture{}.FindReader(r, cfg, func(_ stream.Match[*EmailCaptureBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (EmailCapture) FindReaderFirst(r io.Reader, cfg stream.Config) (*EmailCaptureBytesResult, int64, error) {
	var result *EmailCaptureBytesResult
	var offset int64
	err := EmailCapture{}.FindReader(r, cfg, func(m stream.Match[*EmailCaptureBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = EmailCapture{}.copyBytesResult(m.Result)
...
func (EmailCapture) copyBytesResult(src *EmailCaptureBytesResult) *EmailCaptureBytesResult {
	if src == nil {
		return nil
	}
	dst := &EmailCaptureBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (EmailCapture) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *EmailCaptureBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return EmailCapture{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (EmailCapture) processTransform(data []byte, isEOF bool, onMatch func(match *EmailCaptureBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &EmailCaptureBytesResult{}

	for {
		result, ok := EmailCapture{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (EmailCapture) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &emailCaptureTransformErrReader{err: err}
	}
	...
	return EmailCapture{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *EmailCaptureBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (EmailCapture{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// emailCaptureTransformErrReader is a reader that always returns an error.
type emailCaptureTransformErrReader struct {
	err error
}

func (r *emailCaptureTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (EmailCapture) getCaptureByIndex(m *EmailCaptureBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	case 1:
...
//	})
func (EmailCapture) SelectReader(r io.Reader, pred func(*EmailCaptureBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return EmailCapture{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (EmailCapture) processSelect(data []byte, isEOF bool, pred func(*EmailCaptureBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &EmailCaptureBytesResult{}
...
//	})
func (EmailCapture) RejectReader(r io.Reader, pred func(*EmailCaptureBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return EmailCapture{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (EmailCapture) processReject(data []byte, isEOF bool, pred func(*EmailCaptureBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &EmailCaptureBytesResult{}
...
func (r *EmailCaptureResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *EmailCaptureBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (EmailCapture) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r EmailCaptureResult

	remaining := input
	offset := 0

	for {
		match, ok := EmailCapture{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailCapture) ReplaceAllBytes(input []byte, template string) []byte {
	return EmailCapture{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailCapture) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	for {
		match, ok := EmailCapture{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailCapture) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r EmailCaptureResult
	match, ok := EmailCapture{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailCapture) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r EmailCaptureBytesResult
	match, ok := EmailCapture{}.FindBytesReuse(input, &r)
	if !ok {
...
// EmailCaptureReplaceTemplate holds a pre-compiled replace template for the EmailCapture pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type EmailCaptureReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailCapture) CompileReplaceTemplate(template string) (*EmailCaptureReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &EmailCaptureReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *EmailCaptureReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *EmailCaptureReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r EmailCaptureResult
...
func (t *EmailCaptureReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *EmailCaptureReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r EmailCaptureBytesResult
...
func (t *EmailCaptureReplaceTemplate) ReplaceFirstString(input string) string {
	var r EmailCaptureResult
	match, ok := EmailCapture{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *EmailCaptureReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r EmailCaptureBytesResult
	match, ok := EmailCapture{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
