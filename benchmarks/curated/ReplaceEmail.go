...
var replaceEmailStackPool = sync.Pool{New: func() interface{} {
	stack := make([][3]int, 0, 32)
	return &stack
}}

var replaceEmailCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([]int, 0, 128)
	return &stack
}}

type ReplaceEmail struct{}

var CompiledReplaceEmail = ReplaceEmail{}

// MinMatchLen is the minimum number of bytes any match can have.
const ReplaceEmailMinMatchLen = 5

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const ReplaceEmailMaxMatchLen = -1

func (ReplaceEmail) MatchLengthInfo() (minLen, maxLen int) {
	return ReplaceEmailMinMatchLen, ReplaceEmailMaxMatchLen
}

func (ReplaceEmail) MatchString(input string) bool {
	l := len(input)
	offset := 0
	stackPtr := replaceEmailStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
		for i := range stack {
			stack[i] = [3]int{0, 0, 0}
		...
		*stackPtr = stack[:0]
		replaceEmailStackPool.Put(stackPtr)
	}()
	nextInstruction := 1
	goto StepSelect
TryFallback:
...
func (ReplaceEmail) MatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := replaceEmailStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
...
type ReplaceEmailResult struct {
...
type ReplaceEmailBytesResult struct {
...
func (ReplaceEmail) FindStringReuse(input string, r *ReplaceEmailResult) (*ReplaceEmailResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := replaceEmailCaptureStackPool.Get().(*[]int)
	captureStack := (*captureStackPtr)[:0]
	defer func() {
		*captureStackPtr = captureStack[:0]
		replaceEmailCaptureStackPool.Put(captureStackPtr)
	}()
	stackPtr := replaceEmailStackPool.Get().(*[][3]int)
	stack := (*stackPtr)[:0]
	defer func() {
...
func (recv ReplaceEmail) FindString(input string) (*ReplaceEmailResult, bool) {
	return recv.FindStringReuse(input, nil)
}
func (ReplaceEmail) FindAllStringAppend(input string, n int, s []*ReplaceEmailResult) []*ReplaceEmailResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins15:
		{
			captures[1] = offset
			var item *ReplaceEmailResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReplaceEmailResult{}
...
func (r ReplaceEmail) FindAllString(input string, n int) []*ReplaceEmailResult {
	return r.FindAllStringAppend(input, n, nil)
}
func (ReplaceEmail) FindBytesReuse(input []byte, r *ReplaceEmailBytesResult) (*ReplaceEmailBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captureStackPtr := replaceEmailCaptureStackPool.Get().(*[]int)
...
func (recv ReplaceEmail) FindBytes(input []byte) (*ReplaceEmailBytesResult, bool) {
	return recv.FindBytesReuse(input, nil)
}
func (ReplaceEmail) FindAllBytesAppend(input []byte, n int, s []*ReplaceEmailBytesResult) []*ReplaceEmailBytesResult {
	if n == 0 {
		return s
	}
	result := s
	...
	Ins15:
		{
			captures[1] = offset
			var item *ReplaceEmailBytesResult
			if len(result) < cap(result) {
				result = result[:len(result)+1]
				item = result[len(result)-1]
				if item == nil {
					item = &ReplaceEmailBytesResult{}
					result[len(result)-1] = item
				}
			} else {
				item = &ReplaceEmailBytesResult{}
...
func (r ReplaceEmail) FindAllBytes(input []byte, n int) []*ReplaceEmailBytesResult {
...
func (ReplaceEmail) DefaultMaxLeftover() int {
	return 1048576
}
...
func (ReplaceEmail) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*ReplaceEmailBytesResult]) bool) error {
	...
	reuseResult := &ReplaceEmailBytesResult{}
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := ReplaceEmail{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*ReplaceEmailBytesResult]{
...
func (ReplaceEmail) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := ReplaceEmail{}.FindReader(r, cfg, func(_ stream.Match[*ReplaceEmailBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (ReplaceEmail) FindReaderFirst(r io.Reader, cfg stream.Config) (*ReplaceEmailBytesResult, int64, error) {
	var result *ReplaceEmailBytesResult
	var offset int64
	err := ReplaceEmail{}.FindReader(r, cfg, func(m stream.Match[*ReplaceEmailBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = ReplaceEmail{}.copyBytesResult(m.Result)
...
func (ReplaceEmail) copyBytesResult(src *ReplaceEmailBytesResult) *ReplaceEmailBytesResult {
	if src == nil {
		return nil
	}
	dst := &ReplaceEmailBytesResult{}
	// Copy Match slice
...
//	    })
//	io.Copy(os.Stdout, r)
func (ReplaceEmail) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *ReplaceEmailBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReplaceEmail{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (ReplaceEmail) processTransform(data []byte, isEOF bool, onMatch func(match *ReplaceEmailBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReplaceEmailBytesResult{}

	for {
		result, ok := ReplaceEmail{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (ReplaceEmail) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &replaceEmailTransformErrReader{err: err}
	}
	...
	return ReplaceEmail{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *ReplaceEmailBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (ReplaceEmail{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// replaceEmailTransformErrReader is a reader that always returns an error.
type replaceEmailTransformErrReader struct {
	err error
}

func (r *replaceEmailTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (ReplaceEmail) getCaptureByIndex(m *ReplaceEmailBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	case 1:
...
//	})
func (ReplaceEmail) SelectReader(r io.Reader, pred func(*ReplaceEmailBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReplaceEmail{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (ReplaceEmail) processSelect(data []byte, isEOF bool, pred func(*ReplaceEmailBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReplaceEmailBytesResult{}
...
//	})
func (ReplaceEmail) RejectReader(r io.Reader, pred func(*ReplaceEmailBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return ReplaceEmail{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (ReplaceEmail) processReject(data []byte, isEOF bool, pred func(*ReplaceEmailBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &ReplaceEmailBytesResult{}
...
func (r *ReplaceEmailResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *ReplaceEmailBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (ReplaceEmail) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r ReplaceEmailResult

	remaining := input
	offset := 0

	for {
		match, ok := ReplaceEmail{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReplaceEmail) ReplaceAllBytes(input []byte, template string) []byte {
	return ReplaceEmail{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReplaceEmail) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r ReplaceEmailBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := ReplaceEmail{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReplaceEmail) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReplaceEmailResult
	match, ok := ReplaceEmail{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReplaceEmail) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r ReplaceEmailBytesResult
	match, ok := ReplaceEmail{}.FindBytesReuse(input, &r)
	if !ok {
...
// ReplaceAllString0 replaces all matches with pre-compiled template: $user@REDACTED.$tld
func (ReplaceEmail) ReplaceAllString0(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReplaceEmailResult
...
// ReplaceAllBytes0 replaces all matches with pre-compiled template: $user@REDACTED.$tld
func (ReplaceEmail) ReplaceAllBytes0(input []byte) []byte {
	return ReplaceEmail{}.ReplaceAllBytesAppend0(input, nil)
}

// ReplaceAllBytesAppend0 replaces all matches and appends to buf.
// Pre-compiled template: $user@REDACTED.$tld
func (ReplaceEmail) ReplaceAllBytesAppend0(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReplaceEmailBytesResult
...
// ReplaceFirstString0 replaces only the first match with pre-compiled template: $user@REDACTED.$tld
func (ReplaceEmail) ReplaceFirstString0(input string) string {
	var r ReplaceEmailResult
	match, ok := ReplaceEmail{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
// ReplaceFirstBytes0 replaces only the first match with pre-compiled template: $user@REDACTED.$tld
func (ReplaceEmail) ReplaceFirstBytes0(input []byte) []byte {
	var r ReplaceEmailBytesResult
	match, ok := ReplaceEmail{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
...
// ReplaceAllString1 replaces all matches with pre-compiled template: [EMAIL]
func (ReplaceEmail) ReplaceAllString1(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReplaceEmailResult
...
// ReplaceAllBytes1 replaces all matches with pre-compiled template: [EMAIL]
func (ReplaceEmail) ReplaceAllBytes1(input []byte) []byte {
	return ReplaceEmail{}.ReplaceAllBytesAppend1(input, nil)
}

// ReplaceAllBytesAppend1 replaces all matches and appends to buf.
// Pre-compiled template: [EMAIL]
func (ReplaceEmail) ReplaceAllBytesAppend1(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReplaceEmailBytesResult
...
// ReplaceFirstString1 replaces only the first match with pre-compiled template: [EMAIL]
func (ReplaceEmail) ReplaceFirstString1(input string) string {
	var r ReplaceEmailResult
	match, ok := ReplaceEmail{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
// ReplaceFirstBytes1 replaces only the first match with pre-compiled template: [EMAIL]
func (ReplaceEmail) ReplaceFirstBytes1(input []byte) []byte {
	var r ReplaceEmailBytesResult
	match, ok := ReplaceEmail{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
...
// ReplaceAllString2 replaces all matches with pre-compiled template: $0
func (ReplaceEmail) ReplaceAllString2(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReplaceEmailResult
	...
	for {
		...
		{
			// Optimized: uses only full match, no capture field access
			result.WriteString(match.Match)
		}
		lastEnd = matchEnd
		if len(match.Match) > 0 {
...
// ReplaceAllBytes2 replaces all matches with pre-compiled template: $0
func (ReplaceEmail) ReplaceAllBytes2(input []byte) []byte {
	return ReplaceEmail{}.ReplaceAllBytesAppend2(input, nil)
}

// ReplaceAllBytesAppend2 replaces all matches and appends to buf.
// Pre-compiled template: $0
func (ReplaceEmail) ReplaceAllBytesAppend2(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReplaceEmailBytesResult
...
// ReplaceFirstString2 replaces only the first match with pre-compiled template: $0
func (ReplaceEmail) ReplaceFirstString2(input string) string {
	var r ReplaceEmailResult
	match, ok := ReplaceEmail{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
// ReplaceFirstBytes2 replaces only the first match with pre-compiled template: $0
func (ReplaceEmail) ReplaceFirstBytes2(input []byte) []byte {
	var r ReplaceEmailBytesResult
	match, ok := ReplaceEmail{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
...
// ReplaceEmailReplaceTemplate holds a pre-compiled replace template for the ReplaceEmail pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type ReplaceEmailReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (ReplaceEmail) CompileReplaceTemplate(template string) (*ReplaceEmailReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &ReplaceEmailReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *ReplaceEmailReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *ReplaceEmailReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r ReplaceEmailResult
...
func (t *ReplaceEmailReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *ReplaceEmailReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r ReplaceEmailBytesResult
...
func (t *ReplaceEmailReplaceTemplate) ReplaceFirstString(input string) string {
	var r ReplaceEmailResult
	match, ok := ReplaceEmail{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *ReplaceEmailReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r ReplaceEmailBytesResult
	match, ok := ReplaceEmail{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
