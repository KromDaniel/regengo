...
//	    })
//	io.Copy(os.Stdout, r)
func (MultiEmail) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *MultiEmailBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return MultiEmail{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (MultiEmail) processTransform(data []byte, isEOF bool, onMatch func(match *MultiEmailBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &MultiEmailBytesResult{}

	for {
		result, ok := MultiEmail{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (MultiEmail) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &multiEmailTransformErrReader{err: err}
	}
	...
	return MultiEmail{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *MultiEmailBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (MultiEmail{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// multiEmailTransformErrReader is a reader that always returns an error.
type multiEmailTransformErrReader struct {
	err error
}

func (r *multiEmailTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (MultiEmail) getCaptureByIndex(m *MultiEmailBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	case 1:
...
//	})
func (MultiEmail) SelectReader(r io.Reader, pred func(*MultiEmailBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return MultiEmail{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (MultiEmail) processSelect(data []byte, isEOF bool, pred func(*MultiEmailBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &MultiEmailBytesResult{}
...
//	})
func (MultiEmail) RejectReader(r io.Reader, pred func(*MultiEmailBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return MultiEmail{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (MultiEmail) processReject(data []byte, isEOF bool, pred func(*MultiEmailBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &MultiEmailBytesResult{}
