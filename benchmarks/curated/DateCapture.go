...
const DateCaptureMinMatchLen = 10

// MaxMatchLen is the maximum number of bytes any match can have.
// -1 means unbounded (pattern contains * or + quantifiers).
const DateCaptureMaxMatchLen = 10

func (DateCapture) MatchLengthInfo() (minLen, maxLen int) {
	return DateCaptureMinMatchLen, DateCaptureMaxMatchLen
}
...
func (DateCapture) FindStringReuse(input string, r *DateCaptureResult) (*DateCaptureResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captures[0] = 0
...
func (recv DateCapture) FindString(input string) (*DateCaptureResult, bool) {
...
func (DateCapture) FindBytesReuse(input []byte, r *DateCaptureBytesResult) (*DateCaptureBytesResult, bool) {
	l := len(input)
	offset := 0
	var captures [8]int
	captures[0] = 0
...
func (recv DateCapture) FindBytes(input []byte) (*DateCaptureBytesResult, bool) {
...
func (DateCapture) DefaultMaxLeftover() int {
	return 1024
}
...
func (DateCapture) FindReader(r io.Reader, cfg stream.Config, onMatch func(stream.Match[*DateCaptureBytesResult]) bool) error {
			...
			if err == io.EOF {
				...
				if leftover > 0 {
					...
					for searchPos < len(chunk) {
						result, ok := DateCapture{}.FindBytesReuse(chunk[searchPos:], reuseResult)
						if !ok {
							break
						}
						...
						m := stream.Match[*DateCaptureBytesResult]{
...
func (DateCapture) FindReaderCount(r io.Reader, cfg stream.Config) (int64, error) {
	var count int64
	err := DateCapture{}.FindReader(r, cfg, func(_ stream.Match[*DateCaptureBytesResult]) bool {
		count++
		return true
	})
	return count, err
...
func (DateCapture) FindReaderFirst(r io.Reader, cfg stream.Config) (*DateCaptureBytesResult, int64, error) {
	var result *DateCaptureBytesResult
	var offset int64
	err := DateCapture{}.FindReader(r, cfg, func(m stream.Match[*DateCaptureBytesResult]) bool {
		// Copy the result since buffer will be reused
		result = DateCapture{}.copyBytesResult(m.Result)
...
func (DateCapture) copyBytesResult(src *DateCaptureBytesResult) *DateCaptureBytesResult {
	if src == nil {
		return nil
	}
	dst := &DateCaptureBytesResult{}
	// Copy Match slice
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *DateCaptureBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (DateCapture) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r DateCaptureResult

	remaining := input
	offset := 0

	for {
		match, ok := DateCapture{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (DateCapture) ReplaceAllBytes(input []byte, template string) []byte {
	return DateCapture{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (DateCapture) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r DateCaptureBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := DateCapture{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (DateCapture) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r DateCaptureResult
	match, ok := DateCapture{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (DateCapture) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r DateCaptureBytesResult
	match, ok := DateCapture{}.FindBytesReuse(input, &r)
	if !ok {
...
// DateCaptureReplaceTemplate holds a pre-compiled replace template for the DateCapture pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type DateCaptureReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (DateCapture) CompileReplaceTemplate(template string) (*DateCaptureReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &DateCaptureReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *DateCaptureReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *DateCaptureReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r DateCaptureResult
...
func (t *DateCaptureReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *DateCaptureReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r DateCaptureBytesResult
...
func (t *DateCaptureReplaceTemplate) ReplaceFirstString(input string) string {
	var r DateCaptureResult
	match, ok := DateCapture{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *DateCaptureReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r DateCaptureBytesResult
	match, ok := DateCapture{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
