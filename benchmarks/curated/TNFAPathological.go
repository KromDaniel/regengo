...
//	    })
//	io.Copy(os.Stdout, r)
func (TNFAPathological) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *TNFAPathologicalBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return TNFAPathological{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (TNFAPathological) processTransform(data []byte, isEOF bool, onMatch func(match *TNFAPathologicalBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &TNFAPathologicalBytesResult{}

	for {
		result, ok := TNFAPathological{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (TNFAPathological) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &tNFAPathologicalTransformErrReader{err: err}
	}
	...
	return TNFAPathological{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *TNFAPathologicalBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (TNFAPathological{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// tNFAPathologicalTransformErrReader is a reader that always returns an error.
type tNFAPathologicalTransformErrReader struct {
	err error
}

func (r *tNFAPathologicalTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (TNFAPathological) getCaptureByIndex(m *TNFAPathologicalBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	case 1:
...
//	})
func (TNFAPathological) SelectReader(r io.Reader, pred func(*TNFAPathologicalBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return TNFAPathological{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (TNFAPathological) processSelect(data []byte, isEOF bool, pred func(*TNFAPathologicalBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &TNFAPathologicalBytesResult{}
...
//	})
func (TNFAPathological) RejectReader(r io.Reader, pred func(*TNFAPathologicalBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return TNFAPathological{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (TNFAPathological) processReject(data []byte, isEOF bool, pred func(*TNFAPathologicalBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &TNFAPathologicalBytesResult{}
