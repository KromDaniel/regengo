...
var dateCaptureStackPool = sync.Pool{New: func() interface{} {
	stack := make([][2]int, 0, 32)
	return &stack
}}

func DateCaptureMatchString(input string) bool {
	l := len(input)
	offset := 0
...
func DateCaptureMatchBytes(input []byte) bool {
	l := len(input)
	offset := 0
	stackPtr := dateCaptureStackPool.Get().(*[][2]int)
	stack := (*stackPtr)[:0]
...
func DateCaptureFindString(input string) (*DateCaptureMatch, bool) {
	l := len(input)
	offset := 0
	captures := make([]int, 8)
	stackPtr := dateCaptureStackPool.Get().(*[][2]int)
