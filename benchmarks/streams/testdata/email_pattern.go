...
// Code generated by regengo for pattern: (?P<user>[\w.+-]+)@(?P<domain>[\w.-]+\.\w+)
// DO NOT EDIT.
...
//	    })
//	io.Copy(os.Stdout, r)
func (EmailPattern) NewTransformReader(r io.Reader, cfg stream.TransformConfig, onMatch func(match *EmailPatternBytesResult, emit func([]byte))) io.Reader {
	if cfg.BufferSize == 0 {
	...
	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return EmailPattern{}.processTransform(data, isEOF, onMatch, emitOut)
	}
...
func (EmailPattern) processTransform(data []byte, isEOF bool, onMatch func(match *EmailPatternBytesResult, emit func([]byte)), emitOut func([]byte)) int {
	processed := 0
	reuseResult := &EmailPatternBytesResult{}

	for {
		result, ok := EmailPattern{}.FindBytesReuse(data[processed:], reuseResult)
		if !ok {
			// No more matches
			if isEOF {
...
//	io.Copy(os.Stdout, r)
func (EmailPattern) ReplaceReader(r io.Reader, template string) io.Reader {
	parsed, err := replace.Parse(template)
	if err != nil {
		return &emailPatternTransformErrReader{err: err}
	}
	...
	return EmailPattern{}.NewTransformReader(r, stream.DefaultTransformConfig(), func(m *EmailPatternBytesResult, emit func([]byte)) {
		...
		for _, seg := range resolved {
			...
			case replace.SegmentCaptureIndex:
				capture := (EmailPattern{}).getCaptureByIndex(m, seg.CaptureIndex)
				if capture != nil {
...
// emailPatternTransformErrReader is a reader that always returns an error.
type emailPatternTransformErrReader struct {
	err error
}

func (r *emailPatternTransformErrReader) Read(p []byte) (int, error) {
	return 0, r.err
}

// getCaptureByIndex returns the capture group value by its index.
func (EmailPattern) getCaptureByIndex(m *EmailPatternBytesResult, index int) []byte {
	switch index {
	case 0:
		return m.Match
	case 1:
...
//	})
func (EmailPattern) SelectReader(r io.Reader, pred func(*EmailPatternBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return EmailPattern{}.processSelect(data, isEOF, pred, emitOut)
	}
...
func (EmailPattern) processSelect(data []byte, isEOF bool, pred func(*EmailPatternBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &EmailPatternBytesResult{}
...
//	})
func (EmailPattern) RejectReader(r io.Reader, pred func(*EmailPatternBytesResult) bool) io.Reader {
	cfg := stream.DefaultTransformConfig()
	cfg.MaxLeftover = 1048576

	processor := func(data []byte, isEOF bool, _ stream.TransformFunc, emitOut func([]byte)) int {
		return EmailPattern{}.processReject(data, isEOF, pred, emitOut)
	}
...
func (EmailPattern) processReject(data []byte, isEOF bool, pred func(*EmailPatternBytesResult) bool, emitOut func([]byte)) int {
	processed := 0
	reuseResult := &EmailPatternBytesResult{}
...
func (r *EmailPatternResult) CaptureByIndex(idx int) string {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
// CaptureByIndex returns the capture group value by its 0-based index.
// Index 0 returns the full match, 1+ returns capture groups.
func (r *EmailPatternBytesResult) CaptureByIndex(idx int) []byte {
	switch idx {
	case 0:
		return r.Match
	case 1:
...
func (EmailPattern) ReplaceAllString(input string, template string) string {
	...
	lastEnd := 0
	var r EmailPatternResult

	remaining := input
	offset := 0

	for {
		match, ok := EmailPattern{}.FindStringReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailPattern) ReplaceAllBytes(input []byte, template string) []byte {
	return EmailPattern{}.ReplaceAllBytesAppend(input, template, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailPattern) ReplaceAllBytesAppend(input []byte, template string, buf []byte) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	result := buf[:0]
	lastEnd := 0
	var r EmailPatternBytesResult

	remaining := input
	offset := 0

	for {
		match, ok := EmailPattern{}.FindBytesReuse(remaining, &r)
		if !ok {
			break
		}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailPattern) ReplaceFirstString(input string, template string) string {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r EmailPatternResult
	match, ok := EmailPattern{}.FindStringReuse(input, &r)
	if !ok {
		return input
	}
...
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailPattern) ReplaceFirstBytes(input []byte, template string) []byte {
	tmpl, err := replace.Parse(template)
	if err != nil {
		panic(fmt.Sprintf("regengo: invalid replace template: %v", err))
	}
	...
	var r EmailPatternBytesResult
	match, ok := EmailPattern{}.FindBytesReuse(input, &r)
	if !ok {
...
// EmailPatternReplaceTemplate holds a pre-compiled replace template for the EmailPattern pattern.
// Use CompileReplaceTemplate to create one, then call its Replace methods.
type EmailPatternReplaceTemplate struct {
	original string
...
//
// Template syntax: $0 (full match), $1/$2 (by index), $name (by name), $$ (literal $)
func (EmailPattern) CompileReplaceTemplate(template string) (*EmailPatternReplaceTemplate, error) {
	tmpl, err := replace.Parse(template)
	if err != nil {
		return nil, err
	}
	...
	return &EmailPatternReplaceTemplate{
		original: template,
		segments: resolved,
	}, nil
}
...
func (t *EmailPatternReplaceTemplate) String() string {
	return t.original
}

// ReplaceAllString replaces all matches in input using this compiled template.
func (t *EmailPatternReplaceTemplate) ReplaceAllString(input string) string {
	var result strings.Builder
	lastEnd := 0
	var r EmailPatternResult
...
func (t *EmailPatternReplaceTemplate) ReplaceAllBytes(input []byte) []byte {
	return t.ReplaceAllBytesAppend(input, nil)
}

// ReplaceAllBytesAppend replaces all matches and appends to buf.
// If buf has sufficient capacity, no allocation occurs.
func (t *EmailPatternReplaceTemplate) ReplaceAllBytesAppend(input []byte, buf []byte) []byte {
	result := buf[:0]
	lastEnd := 0
	var r EmailPatternBytesResult
...
func (t *EmailPatternReplaceTemplate) ReplaceFirstString(input string) string {
	var r EmailPatternResult
	match, ok := EmailPattern{}.FindStringReuse(input, &r)
	if !ok {
		return input
...
func (t *EmailPatternReplaceTemplate) ReplaceFirstBytes(input []byte) []byte {
	var r EmailPatternBytesResult
	match, ok := EmailPattern{}.FindBytesReuse(input, &r)
	if !ok {
		return append([]byte{}, input...)
