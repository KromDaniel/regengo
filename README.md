...
err := pattern.FindReader(reader, cfg, func(m stream.Match[*ResultType]) bool {
    // WRONG: savedMatches = append(savedMatches, string(m.Result.Match))
    // This works but creates a copy - which is what you need:
...
|----------|---------------|---------------|
| File > available RAM | ✅ | ❌ |
| Network streams | ✅ | ❌ (must buffer) |
| Small strings (<1MB) | ❌ (overhead) | ✅ |
| Need all matches at once | ❌ | ✅ |
| Processing one match at a time | ✅ | Either |
...
| - | `FindReader(r io.Reader, cfg Config, cb func(Match) bool) error` | Streaming match with callback |
| - | `FindReaderCount(r io.Reader, cfg Config) (int64, error)` | Count matches in stream |
| - | `FindReaderFirst(r io.Reader, cfg Config) (*BytesResult, bool, error)` | First match in stream |
| - | `MatchLengthInfo() (min, max int)` | Pattern match length bounds |
