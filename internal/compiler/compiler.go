...
type Compiler struct {
	...
	parsedReplacers            []*ParsedTemplate   // Parsed and validated replacement templates
}
...
func New(config Config) *Compiler {
	...
	if config.Program != nil {
		...
		// Check if the pattern uses word boundaries (\b or \B)
		compiler.hasWordBoundary = hasWordBoundary(config.Program)
	}
...
func (c *Compiler) Generate() error {
	// Parse and validate replacement templates if any
	if err := c.parseAndValidateReplacers(); err != nil {
		return err
	}
	...
	// Generate isWordChar helper function if the pattern uses word boundaries
	if c.hasWordBoundary {
		c.generateIsWordCharHelper()
	}
...
// parseAndValidateReplacers parses and validates all replacement templates.
// It populates c.parsedReplacers with resolved templates (named refs converted to indices).
// Returns an error with template index if any template is invalid.
func (c *Compiler) parseAndValidateReplacers() error {
	if len(c.config.Replacers) == 0 {
		return nil
	}

	c.parsedReplacers = make([]*ParsedTemplate, len(c.config.Replacers))

	// Determine capture count (NumCap includes implicit captures, we want user captures)
	// NumCap counts submatches (2 per capture), so divide by 2 and subtract 1 for full match
	numCaptures := 0
	if c.config.Program != nil {
		numCaptures = (c.config.Program.NumCap / 2) - 1
		if numCaptures < 0 {
			numCaptures = 0
		}
	}
...
// generateIsWordCharHelper generates the isWordChar helper function for word boundary assertions.
// This function checks if a byte is a word character: [a-zA-Z0-9_]
func (c *Compiler) generateIsWordCharHelper() {
	c.file.Comment("isWordChar returns true if the byte is a word character [a-zA-Z0-9_]")
	c.file.Func().Id(codegen.IsWordCharName).Params(jen.Id("b").Byte()).Bool().Block(
		jen.Return(
			jen.Parens(jen.Id("b").Op(">=").Lit(byte('a')).Op("&&").Id("b").Op("<=").Lit(byte('z'))).
				Op("||").
				Parens(jen.Id("b").Op(">=").Lit(byte('A')).Op("&&").Id("b").Op("<=").Lit(byte('Z'))).
				Op("||").
				Parens(jen.Id("b").Op(">=").Lit(byte('0')).Op("&&").Id("b").Op("<=").Lit(byte('9'))).
				Op("||").
				Id("b").Op("==").Lit(byte('_')),
		),
	)
